...
import './ManageMemoryOptimizedTableTypesDialog.css'
import BaseDialog from '../../../../components/BaseDialog/BaseDialog'
import ErrorBox from '../../../../components/ErrorBox/ErrorBox'
import Button from '../../../../components/Button/Button'

interface ManageMemoryOptimizedTableTypesDialogProps {
  connectionId: string
  databaseName: string
...
export default function ManageMemoryOptimizedTableTypesDialog({
  connectionId,
  databaseName,
  initialTypeName,
  openOnNew,
  onClose,
  onSuccess
}: ManageMemoryOptimizedTableTypesDialogProps): React.JSX.Element {
  ...
  const loadTableTypes = useCallback(async () => {
    ...
    try {
      const result = await window.api.database.getMemoryOptimizedTableTypes(connectionId, databaseName)
      if (result.status === 'ok') {
  ...
  async function selectTableType(tt: TypeListItem): Promise<void> {
    ...
    try {
      const result = await window.api.database.getMemoryOptimizedTableType(
        connectionId,
        databaseName,
        tt.schemaName,
        tt.typeName
  ...
  async function handleSave(): Promise<void> {
    if (!editState.typeName.trim()) {
      setError(t('explorer.manageMemoryOptimizedTableTypes.validation.nameRequired'))
      return
    }
    if (!editState.schemaName) {
      setError(t('explorer.manageMemoryOptimizedTableTypes.validation.schemaRequired'))
      return
    }
    const hasPrimaryKey = editState.columns.some((c) => c.isPrimaryKey)
    if (!hasPrimaryKey) {
      setError(t('explorer.manageMemoryOptimizedTableTypes.validation.primaryKeyRequired'))
      return
    }
    ...
    try {
      ...
      const result = await window.api.database.saveMemoryOptimizedTableType(
        connectionId,
        databaseName,
        params,
        origName,
  ...
  async function handleDelete(): Promise<void> {
    ...
    try {
      ...
      const result = await window.api.database.deleteMemoryOptimizedTableType(
        connectionId,
        databaseName,
        schemaName,
        typeName
  ...
  const mottFooter = showEditor ? (
    <>
      <div className="mott-dialog__error-area">{error && <ErrorBox error={error} />}</div>
      <div className="dialog__footer-right">
        {hasSelection && (
          ...
          >
            <Trash2 size={13} />
            {t('explorer.manageMemoryOptimizedTableTypes.deleteButton')}
          </Button>
        )}
        <Button
              variant="primary"
        ...
        >
          {isAddingNew
            ? t('explorer.manageMemoryOptimizedTableTypes.saveButton')
            : t('explorer.manageMemoryOptimizedTableTypes.updateButton')}
        </Button>
      </div>
    </>
  ) : undefined
  ...
  return (
    <BaseDialog
      title={t('explorer.manageMemoryOptimizedTableTypes.dialogTitle')}
      icon={<Tag size={16} />}
      onClose={onClose}
      width="90vw"
      maxWidth="900px"
      ...
      footer={mottFooter}
    >
        <div className="mott-dialog__body">
          {/* Left panel: list */}
          <div className="mott-dialog__list-panel">
            <div className="mott-dialog__list-header">
              {t('explorer.manageMemoryOptimizedTableTypes.listHeader')}
            </div>
            <div className="mott-dialog__list">
              {loadingTypes ? (
                <div className="mott-dialog__empty-state">
                  {t('common.loading', 'Loading…')}
                </div>
              ) : tableTypes.length === 0 && !isAddingNew ? (
                <div className="mott-dialog__empty-state mott-dialog__empty-state--list">
                  {t('explorer.manageMemoryOptimizedTableTypes.noItems')}
                </div>
              ) : (
                <>
                  {tableTypes.map((tt) => {
                    ...
                    return (
                      <div
                        key={key}
                        className={`mott-dialog__list-item${
                          selectedKey === key && !isAddingNew
                            ? ' mott-dialog__list-item--selected'
                            : ''
                        }`}
                  ...
                  {isAddingNew && (
                    <div className="mott-dialog__list-item mott-dialog__list-item--selected">
                      <Tag size={12} style={{ flexShrink: 0 }} />
                      {editState.typeName
                        ? `${editState.schemaName}.${editState.typeName}`
                        : t('explorer.manageMemoryOptimizedTableTypes.newItem')}
                    </div>
                  )}
                </>
              )}
            </div>
            <button className="mott-dialog__list-add" onClick={startAddNew} disabled={isBusy}>
              <Plus size={13} />
              {t('explorer.manageMemoryOptimizedTableTypes.addButton')}
            </button>
          </div>

          {/* Right panel: editor */}
          <div className="mott-dialog__editor-panel">
            {!showEditor ? (
              <div className="mott-dialog__empty-state">
                {t('explorer.manageMemoryOptimizedTableTypes.selectOrAdd')}
              </div>
            ) : (
              <>
                {/* Schema + Name row */}
                <div className="mott-dialog__editor-header">
                  <div className="mott-dialog__field">
                    <label className="mott-dialog__label" htmlFor="mott-schema">
                      {t('explorer.manageMemoryOptimizedTableTypes.schemaLabel')}
                    </label>
                    <select
                      id="mott-schema"
                      className="mott-dialog__select"
                  ...
                  <div className="mott-dialog__field">
                    <label className="mott-dialog__label">
                      {t('explorer.manageMemoryOptimizedTableTypes.nameLabel')}
                    </label>
                    <input
                      className="mott-dialog__input"
                      type="text"
                      value={editState.typeName}
                      onChange={(e) =>
                        setEditState((prev) => ({ ...prev, typeName: e.target.value }))
                      }
                      placeholder={t('explorer.manageMemoryOptimizedTableTypes.namePlaceholder')}
                      disabled={!isAddingNew || isBusy}
                ...
                {/* Columns editor */}
                {loadingColumns ? (
                  <div className="mott-dialog__columns-loading">
                    {t('common.loading', 'Loading…')}
                  </div>
                ) : (
                  <TableColumnsEditor
