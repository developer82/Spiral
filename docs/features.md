...
- **Take Screenshot shortcut** — **Cmd+Shift+T** (macOS) / **Ctrl+Shift+T** (Windows & Linux) takes an **instant screenshot** without opening the dialog: it captures the window at its **current** size (no resize) and goes straight to the PNG save dialog, applying the same macOS traffic-light overlay. It is handled in the renderer (`TopBar` window `keydown`, matched on `event.code === 'KeyT'` plus the platform-appropriate primary modifier + Shift) rather than as a native menu accelerator, so it never opens the dialog the Help-menu item opens. The Take Screenshot dialog also shows a small hint under the preview advertising this shortcut (platform-aware Cmd/Ctrl).
